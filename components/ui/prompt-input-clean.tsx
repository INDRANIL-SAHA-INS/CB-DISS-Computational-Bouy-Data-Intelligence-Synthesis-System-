...
export const PromptBox = React.forwardRef<HTMLTextAreaElement, PromptBoxProps>(
  ({ className, onSubmit, value: externalValue, onChange, showCollapseButton = false, isCollapsed = false, onToggleCollapse, isLoading = false, onStopRequest, ...props }, ref) => {
    ...
    return (
      <div
        className={cn(
          "relative rounded-xl transition-all duration-300 bg-slate-800 backdrop-blur-md border border-slate-700 overflow-hidden shadow-lg w-full max-w-3xl mx-auto",
          className
        )}
      >
        <input
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-300 hover:text-white hover:bg-gray-700 transition-all duration-200 group"
                >
  ...
  <div className="absolute bottom-3 right-3 flex items-center gap-2 z-10">
                ...
                {toolsList.map((tool) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-3 px-3 py-2 text-white hover:bg-gray-700 rounded-lg transition-colors"
                    >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-lg text-red-500 hover:text-white hover:bg-red-600 transition-all duration-200 group z-20"
        ...
        <textarea
          ...
          className="w-full resize-none border-0 bg-transparent pl-14 pr-14 py-2 text-white placeholder:text-gray-400 focus:ring-0 focus-visible:outline-none min-h-[40px] text-base font-medium"
