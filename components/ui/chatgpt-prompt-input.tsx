...
const SendIcon = (props: React.SVGProps<SVGSVGElement>) => ( <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}><path d="M7 11L12 6L17 11M12 18V7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg> );
...
export const PromptBox = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div className={cn("chat-input-glow rounded-xl transition-all duration-300 bg-[#2B2B2B]/90 backdrop-blur-md border border-[#3A3A3A]/50 overflow-hidden shadow-2xl shadow-black/30", className)}>
        <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*"/>
        
        {/* Tools Popover */}
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
          <PopoverContent side="top" className="p-2 bg-[#2B2B2B] border-[#3A3A3A] shadow-xl">
              ...
              {toolsList.map((tool) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center gap-3 px-3 py-2 text-[#E0E0E0] hover:bg-[#3A3A3A]/50 rounded-lg transition-colors duration-200"
                  >
                    <Icon className="h-5 w-5" />
                    <div className="flex flex-col items-start">
                      <span className="text-sm font-medium">{tool.name}</span>
                      {tool.extra && (
                        <span className="text-xs text-[#8A8A8A]">{tool.extra}</span>
                      )}
                    </div>
                  </button>
                );
        ...
        {imagePreview && ( 
          <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}> 
            <div className="relative mb-1 w-fit rounded-[1rem] px-1 pt-1"> 
              <button type="button" className="transition-transform relative w-[58px] h-[58px]" onClick={() => setIsImageDialogOpen(true)}> 
                <Image 
                  src={imagePreview} 
                  alt="Image preview" 
        ...
        <div className="flex items-center">
          <div className="flex-1">
            <textarea ref={internalTextareaRef} rows={1} value={value} onChange={handleInputChange} placeholder="Ask me about the ocean..." className="custom-scrollbar w-full resize-none border-0 bg-transparent px-6 py-4 text-[#E0E0E0] placeholder:text-[#8A8A8A] focus:ring-0 focus-visible:outline-none min-h-[56px] text-base font-normal" {...props} />
          </div>
          ...
          <div className="flex items-center px-4 gap-2">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button 
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-lg text-[#8A8A8A] hover:text-[#007B8C] hover:bg-[#3A3A3A]/50 transition-all duration-200 focus-visible:outline-none group"
                  >
                    <ToolsIcon className="h-5 w-5 group-hover:scale-110 transition-transform duration-200" />
                    <span className="sr-only">Tools</span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="top" showArrow={true} className="bg-[#2B2B2B] text-[#E0E0E0] border-[#3A3A3A]">
                  <p>Tools</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button type="button" className="flex h-10 w-10 items-center justify-center rounded-lg text-[#8A8A8A] hover:text-[#007B8C] hover:bg-[#3A3A3A]/50 transition-all duration-200 focus-visible:outline-none group">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button 
                    type="submit"
                    className={cn(
                      "flex h-10 w-10 items-center justify-center rounded-lg transition-all duration-200 focus-visible:outline-none group",
                      hasValue 
                        ? "text-white bg-[#007B8C] hover:bg-[#006B7C]" 
                        : "text-[#8A8A8A] hover:text-[#007B8C] hover:bg-[#3A3A3A]/50"
                    )}
                    disabled={!hasValue}
                  >
